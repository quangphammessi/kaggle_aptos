...
# model.load_state_dict(torch.load('./saved_model/20190821_newolddata_effib4.pt'))
model.cuda()
...
radam_base = RAdam(model.parameters(), lr=lr, weight_decay=1e-5)
optimizer = Lookahead(radam_base, k=5, alpha=0.5)
...
reducelr = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.4, patience=4, verbose=True, min_lr=1e-6)
...
print('transforms.RandomAffine(degrees=360, translate=(0.05, 0.05), scale=(1, 1.3)), \
    transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1), \
    transforms.RandomRotation((-120, 120))')
print('RAdam + Lookahead')
print('Model: 20190901_data3k_effib4_aug_clean_radam.pt')
...
for epoch in range(n_epochs):
    ...
    #     torch.save(model.state_dict(), './saved_model/20190901_data3k_effib4_aug_clean_radam.pt')
