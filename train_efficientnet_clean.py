...
ADAPTIVE_REGRESSION_LOSS = './robust_loss_pytorch/'
sys.path.append(package_path)
sys.path.append(NVIDIA_apex_path)
sys.path.append(WARMUP_LR)
sys.path.append(ADAPTIVE_REGRESSION_LOSS)
...
train_csv = pd.read_csv('./data/clean_train.csv')
...
# model.load_state_dict(torch.load('./saved_model/20190902_newolddata_effib4_aug_clean.pt'))
model.cuda()
...
criterion = AdaptiveLossFunction(num_dims=1, float_dtype=torch.float32, device='cuda:0')
...
def train_model(epoch):
    ...
    for idx, (imgs, labels) in enumerate(train_loader):
        ...
        loss = torch.mean(criterion.lossfun(output_train - labels_train))
...
def test_model():
    ...
    with torch.no_grad():
        for idx, (imgs, labels) in enumerate(val_loader):
            ...
            avg_val_loss += torch.mean(criterion.lossfun(output_test - labels_vaild)).data / len(val_loader)
...
print('transforms.RandomAffine(degrees=360, translate=(0.05, 0.05), scale=(1, 1.3)), \
    transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1), \
    transforms.RandomRotation((-120, 120))')
print('Model: 20190904_data3k_effib4_aug_clean_adaptive_loss.pt')
...
for epoch in range(n_epochs):
    ...
    if avg_val_loss < best_avg_loss:
        ...
        torch.save(model.state_dict(), './saved_model/20190904_data3k_effib4_aug_clean_adaptive_loss.pt')
