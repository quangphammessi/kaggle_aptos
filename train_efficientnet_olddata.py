...
train_2015 = '/media/asilla/data123/quangpn/APTOS2019/newtrain2015/'

# train_csv = pd.read_csv('./data/train.csv')
submission_csv = pd.read_csv('./data/sample_submission.csv')

train_2019_csv = pd.read_csv('./data/clean_train.csv')
# train_2015_csv = pd.read_csv('./data/newtrain2015.csv')
train_2015_csv = pd.read_csv('./data/newtrain2015.csv', usecols=['id_code', 'diagnosis'])[['id_code', 'diagnosis']]


# # Chia tap data 2019 thanh train va val set sau do hoc theo 
# train_df, val_df = train_test_split(train_2019_csv, test_size=0.1, random_state=2018, stratify=train_2019_csv.diagnosis)
# train_df.reset_index(drop=True, inplace=True)
# val_df.reset_index(drop=True, inplace=True)


train_df = train_2015_csv
val_df = train_2019_csv
...
scheduler_step = torch.optim.lr_scheduler.StepLR(optimizer, step_size=8, gamma=0.1)
...
print('transforms.RandomAffine(degrees=360, translate=(0.05, 0.05), scale=(1, 1.3)), \
    transforms.ColorJitter(brightness=0.5, contrast=0.5, saturation=0.5, hue=0.1), \
    transforms.RandomRotation((-120, 120))')
print('Model: 20190902_newolddata_effib4_aug_clean.pt')
...
for epoch in range(n_epochs):
    ...
    if avg_val_loss < best_avg_loss:
        ...
        torch.save(model.state_dict(), './saved_model/20190902_newolddata_effib4_aug_clean.pt')
        print('Better val_loss. Model saved!')

    # if val_acc > best_val_acc:
    #     best_val_acc = val_acc
    #     torch.save(model.state_dict(), './saved_model/20190810_data3k_effi_b4.pt')
